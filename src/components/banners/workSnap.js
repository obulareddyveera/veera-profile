...
const WorkSnap = () => {
  const getMonthYear = (date) => {
    return (
      <div>
        {date ? (
          <>
            <span className="text-gray-700 font-dancingScript font-bold">
              {moment(date).format("MMM")}{" "}
            </span>
            <span className="text-gray-800 font-dancingScript font-bold">
              {moment(date).format("YYYY")}
            </span>
          </>
        ) : (
          <span className="text-gray-900 font-dancingScript font-bold">
            Till Date
          </span>
        )}
      </div>
  ...
  const getTotalExpPerSlot = (startDate, endDate) => {
    ...
    return (
      <div>
        <span className="text-gray-500 font-dancingScript font-bold">
          {Math.round((current.diff(given, "months") / 12) * 10) / 10}
        </span>
        <span className="text-gray-400 font-dancingScript font-bold">
          {" "}
          years
        </span>
      </div>
  ...
  const getTotalExp = (payload) => {
    ...
    return (
      <div>
        <span className="text-gray-500 font-dancingScript font-bold">
          {Math.round((totalExpByMonth / 12) * 10) / 10}
        </span>
        <span className="text-gray-400 font-dancingScript font-bold">
          {" "}
          years
  ...
  return (
    <div className="flex flex-col m-4">
      {Config &&
        Config.workExp.map((entity, index) => {
          return (
            <>
              <div
                key={`workSnap${index}`}
                  ...
                  <div className="flex flex-col">
                    <div className="flex">
                      ...
                      <div className="flex flex-col md:flex-row justify-around mb-2">
                        <div className="text-xl font-bold font-merriweather mt-1 ml-1">
                          {entity.name},
                        </div>
                        <div className="text-sm font-bold font-merriweather mt-2 ml-2">
                          {entity.role}
                        </div>
                      </div>
                    </div>
